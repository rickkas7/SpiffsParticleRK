 ...
 *      Author: petera
 *
 *  Retrieved from https://github.com/pellepl/spiffs/
 *  Jume 8, 2018
 *
 *  License: MIT
 */
...
#define SPIFFS_ERR_OUT_OF_MEMORY		-10060
 ...
 */
s32_t SPIFFS_truncate(spiffs *fs, const char *path, s32_t len);
 ...
 * Note: In POSIX, len can be larger than the file size to make the file larger, but this
 * does not work in SPIFFS. len must be less than or equal to the file size.
 * https://github.com/pellepl/spiffs/issues/107
 */
s32_t SPIFFS_ftruncate(spiffs *fs, spiffs_file fh, s32_t len);
